...
## 5. Criar o primeiro login de admin
...
`atendimento.html` é o questionário guiado (área → situação → urgência →
contato) linkado no botão principal do hero e no "Fale Conosco" do menu. Ele
envia para `POST /api/leads`, que grava no D1. O formulário simples da seção
`#contato` do `index.html` continua funcionando em paralelo, via Formspree
(não depende do D1).
